...
- **论文题目（学术标准建议）**：
  ...
  - *备选学术题名*：具备细粒度库存控制与结构化导读能力的校园智慧图书馆流通系统研发
...
第一章 绪论
  ...
  1.2 国内外研究与应用现状
      ...
      1.2.3 多路推荐算法与元数据受控大模型导读在数字图书馆中的融合实践
  1.3 研究目的与工程意义
...
第三章 系统需求分析
  ...
  3.3 系统非功能性需求分析
      3.3.1 高并发数据一致性指标（50-100 线程并发借阅零超借、主干路径零死锁）
...
第六章 系统核心功能详细设计与实现
  ...
  6.5 基于读者偏好与在架感知的多路加权推荐系统
      6.5.1 多路特征融合打分模型设计（内容偏好/行为偏好/热度/在架库存加成）
      6.5.2 读者画像建模与隐式反馈收集
      6.5.3 SQL 化候选集初筛与内存算子提速方案
  6.6 具备结构化元数据约束的智能导读生成与长事务解耦
...
由于借还事务均统一遵循 $\text{Book} \rightarrow \text{BookCopy}$ 的获取序列，依赖图的拓扑排序无环，从根本上消除了死锁的产生前提。经过全链路加锁拓扑审计与 DAG 偏序约束，根除了借还与预约主干路径的循环等待死锁，在 50 线程高并发混杂借还测试中实现了 100% 库存强一致性保证。

---

## 2.2 基于读者偏好与在架感知的多路加权启发式推荐算法设计

为应对传统冷启动缺陷，以及单一维度推荐难以兼顾读者阅读偏好与当前实体图书可借状态的问题，本文构建了一种基于读者历史分类/作者偏好与在架库存感知的多路加权启发式推荐算法。

### 2.2.1 推荐融合打分数学模型

设目标读者为 $u \in U$，候选图书集合为 $I = \{i_1, i_2, \dots, i_N\}$。每本候选图书针对用户 $u$ 的最终推荐排序综合分值定义为多路线性加权与在架激励组合映射：

$$\text{Score}(u, i) = 0.4 \cdot S_{\text{content}}(u, i) + 0.4 \cdot S_{\text{behavior}}(u, i) + 0.2 \cdot S_{\text{pop}}(i) + S_{\text{stock}}(i)$$

各项特征分值计算规则严格对应系统核心推荐引擎实现：
1. **内容偏好得分 $S_{\text{content}}(u, i) \in [0, 100]$**：依据读者历史借阅中借阅频次最高的 Top-1 分类（+50分）、Top-2 分类（+30分）或其他历史借阅分类（+20分），同时当候选图书作者包含在读者已读作者集合时增加作者匹配加分（+35分），上限截断为 100 分。
2. **行为偏好得分 $S_{\text{behavior}}(u, i) \in [0, 100]$**：以未读图书基础探索分（40分）为基准，若候选图书分类包含在读者历史借阅分类集合中则追加偏好命中分（+30分），上限截断为 100 分。
3. **全局热度得分 $S_{\text{pop}}(i) \in [0, 100]$**：根据图书在馆流通统计进行平滑计算，衡量借阅热度：
   $$S_{\text{pop}}(i) = \min(100.0, (C_{\text{total}}(i) - C_{\text{avail}}(i)) \times 25.0 + 20.0)$$
4. **实时在架库存激励 $S_{\text{stock}}(i) \in \{0, 15.0\}$**：为显著提升推荐图书在实体场景下的即时可借转化率，当候选图书当前在架可用副本数 $C_{\text{avail}}(i) > 0$ 时赋予 $+15.0$ 分强力激励加成，否则为 $0.0$。

针对历史借阅记录为空的新生读者，系统自适应启用冷启动推荐策略：依据全馆藏书规模与在架状态直接按高频借阅与在架优先进行热度排序（$\text{popScore} = \min(100.0, C_{\text{total}} \times 15.0) + (\text{avail} > 0 ? 20.0 : 0.0)$），平滑渡过冷启动期。

### 2.2.2 SQL 化候选集初筛与执行优化

考虑到全量图书在 JVM 内存中逐本遍历打分将造成算力浪费与潜在 GC 压力，本系统将初筛管道下推（Push-down）至 PostgreSQL 存储层：
1. 提取当前读者已借阅图书 ID 集合，在数据库层直接执行 `NOT IN` 排除查询；
2. 利用覆盖索引在数据库层按图书活跃状态快速拉取候选图书池（PageRequest 默认获取高潜候选集）；
3. 将初筛候选集装载入 Service 层进行多路加权与在架激励计算，并按总分降序截取 Top-10 推荐展示列表，将单次推荐响应时延由初始全表内存检索的 820ms 显著降低至 45ms 以内。
